...
public abstract class AbstractIntegrationTest {
	...
	protected String getJsonAsString(final String path) throws UnsupportedEncodingException, FileNotFoundException {
        final InputStream inputStream = getClass().getResourceAsStream(String.format("/br/gov/sp/fatec/mapskills/account/%s", path));
        if (inputStream == null) {
            throw new FileNotFoundException("File " + path + " not found. A file named " + path + " must be present "
                    + "in the src/test/resources/br/gov/sp/fatec/mapskills/account/ folder of the project whose class matches being tested.");
        }
	...
	protected void verifyDatasetForTable(final String fileName, final String tableName, final String sqlStatement, final String[] ignoredColumns)
			throws Exception {
		assertEqualsByQuery(getDataSet("/br/gov/sp/fatec/mapskills/account/database/datasets/" + fileName), getConnection(), sqlStatement, tableName, ignoredColumns);
	}
	...
	protected String getFileAsBase64(final String fileName) throws Exception {
		final InputStream stream = ClassLoader.getSystemResourceAsStream("br/gov/sp/fatec/mapskills/account/file/" + fileName);
		if (stream == null) {
			throw new FileNotFoundException(
					"File " + fileName + " not found. A file named " + fileName + " must be present in the src/test/resources/br/gov/sp/fatec/mapskills/account/file folder of the project whose class is being tested.");
		}		
		return Base64.getEncoder().encodeToString(IOUtils.toByteArray(stream));
	}

	private static Properties loadProperties() throws Exception {
		final InputStream stream = ClassLoader.getSystemResourceAsStream("br/gov/sp/fatec/mapskills/account/config/datasource.properties");
		if (stream == null) {
			throw new FileNotFoundException(
	...
	private String getClearDatabaseFile() {
		return "/br/gov/sp/fatec/mapskills/account/database/clear-database.sql";
	}

	private String getResetSequencesFile() {
		return "/br/gov/sp/fatec/mapskills/account/database/reset-sequences.sql";
	}

	private String getDatabaseFile() {
		return "/br/gov/sp/fatec/mapskills/account/database/database-setup.sql";
	}

	private String getDataFile() {
		return "/br/gov/sp/fatec/mapskills/account/database/data.sql";
	}
}